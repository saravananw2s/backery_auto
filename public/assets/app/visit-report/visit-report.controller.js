(function() {
    ...
    angular.module('app')
        .controller('VisitCtrl', ['$rootScope', '$state', '$stateParams','$mdToast', '$timeout', '$scope', '$mdEditDialog', '$q', '$http', '$mdDialog', 'api', VisitCtrl])


    function VisitCtrl($rootScope, $state, $stateParams, $mdToast, $timeout, $scope, $mdEditDialog, $q, $http, $mdDialog, api) {
        ...
        $scope.view = function(id) {
            ...
            $state.go('admin.visitview', { id: api.encode(id) });
        }
        ...
        $scope.addNewChoice = function(b = '') {
            ...
            var newItemNo = $scope.data.reps_info.length + 1;

            $scope.data.reps_info.push({ comp_id: 0, product: '', rsp: '', wsp: '', stk: '' });
        ...
        $scope.removeChoice = function(data) {
            ...
            if (data > -1) {
               $scope.data.reps_info.splice(data, 1);
            }
        ...
        $scope.update = function() {
                ...
                $http.put($scope.apipath + $scope.data.report_id, { data: $scope.data })
                    .then(
                        function(response) {
                ...
                .then(
                    function(response) {
                        ...
                        if (res.data.status == 'ok') {
                            ...
                            $scope.data.reps_info = angular.fromJson($scope.data.rep_cmp);
