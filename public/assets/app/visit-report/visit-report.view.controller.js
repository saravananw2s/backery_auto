(function() {
    ...
    angular.module('app')
        .controller('VisitViewCtrl', ['$rootScope', '$sce', '$stateParams', '$state', '$timeout', '$scope', '$mdEditDialog', '$q', '$http', '$mdDialog', 'api', VisitViewCtrl])


    function VisitViewCtrl($rootScope, $sce, $stateParams, $state, $timeout, $scope, $mdEditDialog, $q, $http, $mdDialog, api) {
        $scope.id = null;
        $scope.hide = true;
        $scope.apipath = '';
        $scope.data = {};
        ...
        $scope.data.rep_info = {};
        $scope.title = 'Customer Analysis Info';
        $scope.parentState = 'admin.visit';
                ...
                .then(
                    function(response) {
                        ...
                        if (res.data.status == 'ok' && res.data.data != null) {
                            ...
                            temp().then().then(function(value) {
                                $scope.data.user_id =  value.data.first_name + ' ' + value.data.last_name;
                            });

                            var temp = api.getcustomer_name($scope.data.cust_id);
                            temp().then().then(function(value) {
                                $scope.data.cust_id = value.data.name;
                            });

                            $scope.rep_cmp = $scope.data.rep_cmp;
