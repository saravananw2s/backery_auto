(function() {
    'use strict';
    angular.module('app').controller('CustomerStatsCtrl', ['$rootScope', '$state', '$stateParams', '$mdToast', '$timeout', '$scope', '$mdEditDialog', '$q', '$http', '$mdDialog', 'api', CustomerStatsCtrl])

    function CustomerStatsCtrl($rootScope, $state, $stateParams, $mdToast, $timeout, $scope, $mdEditDialog, $q, $http, $mdDialog, api) {
        $scope.date = {
        ...
        $scope.view = function(id) {
            $state.go('admin.customer-statsview', {
                id: api.encode(id)
            });
        }
        ...
        $scope.update_info = function(dataid) {
            ...
            if ($scope.data_form.$dirty) {
                var confirm = $mdDialog.confirm().title('You will lose unsaved changes if you do this action ?').targetEvent(ev).ok('OK, Do it').cancel('Cancel');
                $mdDialog.show(confirm).then(function() {
                    $http.get($scope.apipath + dataid + '?info=1').then(function(response) {
                        var res = angular.fromJson(response);
                        if (res.data.status == 'ok') {
                            ...
                            $scope.data.last_visited = new Date($scope.data.last_visited);
                        } else {}
                    }, function(response) {
                        // failure call back
                    });
