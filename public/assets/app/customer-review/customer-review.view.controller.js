(function() {
    ...
    angular.module('app')
        .controller('CustomerReviewViewCtrl', ['$rootScope', '$sce', '$stateParams', '$state', '$timeout', '$scope', '$mdEditDialog', '$q', '$http', '$mdDialog', 'api', CustomerViewCtrl])
    ...
    function CustomerViewCtrl($rootScope, $sce, $stateParams, $state, $timeout, $scope, $mdEditDialog, $q, $http, $mdDialog, api) {
        ...
        $scope.data = {};
        $scope.title = 'Review';
        $scope.parentState = 'admin.customer-review';
        ...
        if ($stateParams.id) {
            ...
            $scope.hide = false;
            $scope.apipath = 'customer-reviews/' + $scope.id;
            $scope.loaddata();
        }
