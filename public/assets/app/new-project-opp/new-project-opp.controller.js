(function() {
    ...
    angular.module('app')
        .controller('NewProjectOppCtrl', ['$rootScope', '$state', '$stateParams','$mdToast', '$timeout', '$scope', '$mdEditDialog', '$q', '$http', '$mdDialog', 'api', NewProjectOppCtrl])


    function NewProjectOppCtrl($rootScope, $state, $stateParams, $mdToast, $timeout, $scope, $mdEditDialog, $q, $http, $mdDialog, api) {
        ...
        $scope.view = function(id) {
            ...
            $state.go('admin.newprojectoppview', { id: api.encode(id) });
        }
        ...
        $scope.title = 'Lead Opportunity';
        $scope.apipath = 'newprojectopp/';
        $scope.showform = false;
        $scope.add = true;
        $scope.data = {};
        $scope.prospect = [];
  ...
  $scope.products = [];
        ...
        $scope.addNewChoice = function(b = '') {
            ...
            $scope.data.products.push({ product_id: 0, quantity: '', });
        ...
        $scope.removeChoice = function(data) {
            ...
            if (data > -1) {
               $scope.data.products.splice(data, 1);
            }
                ...
                .then(
                    function(response) {
                        ...
                        if (res.data.status == 'ok') {
                            ...
                            $scope.data.products = angular.fromJson($scope.data.products);
                            $scope.data.exp_date = new Date($scope.data.exp_date);
