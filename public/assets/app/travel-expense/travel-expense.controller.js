(function() {
    ...
    angular.module('app')
        .controller('TravelExpenseCtrl', ['$rootScope', '$state', '$stateParams','$mdToast', '$timeout', '$scope', '$mdEditDialog', '$q', '$http', '$mdDialog', 'api', TravelExpenseCtrl])


    function TravelExpenseCtrl($rootScope, $state, $stateParams, $mdToast, $timeout, $scope, $mdEditDialog, $q, $http, $mdDialog, api) {
        ...
        $scope.view = function(id) {
            ...
            $state.go('admin.travelexpenseview', { id: api.encode(id) });
        }
        ...
        $scope.title = 'Travel Expense';
        $scope.apipath = 'travelexpense/';
        $scope.showform = false;
        $scope.add = true;
        $scope.data = {};
        $scope.activity = [];
        ...
        $scope.addNewChoice = function(b = '') {
            ...
            $scope.data.hotel_exp.push({ check_in: '', check_out: '',total_exp: 0, hotel_info: '',desc : '' });
        ...
        $scope.removeChoice = function(data) {
            ...
            if (data > -1) {
               $scope.data.hotel_exp.splice(data, 1);
            }

        };


  $scope.addNewChoice1 = function(b = '') {


            $scope.data.travel_exp.push({ mode: '', start: '', end: '' , total_exp: '', total_kms: '',desc:''});


        };

        $scope.removeChoice1 = function(data) {

            console.log(data);

            if (data > -1) {
               $scope.data.travel_exp.splice(data, 1);
            }
