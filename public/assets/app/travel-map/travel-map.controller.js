(function() {
    'use strict';
    angular.module('app').controller('TravelMapCtrl', ['$rootScope', '$state', '$stateParams', '$mdToast', '$timeout', '$scope', '$interval', '$q', '$http', '$mdDialog', 'api', TravelMapCtrl])

    function TravelMapCtrl($rootScope, $state, $stateParams, $mdToast, $timeout, $scope, $interval, $q, $http, $mdDialog, api) {
 ...
 $scope.tempcus = api.customers();
        ...
        $scope.filterdata = function() {
            ...
            var data = _.find($scope.users, function(item) {
                              if (item.user_id == $scope.emp_id) {
                    return true;
                } else {
                    return false;
                }
            ...
            if (data) {
                     ...
                     $scope.ToggleTrackingView(data,'emp');
            }
                ...
                //     $http.get('location_emp_api/' + $scope.emp_id, {
                //                 data: {
                //                     start_date: $scope.datePicker,
                //                     end_date: $scope.datePicker,
                //                     emp_id: $scope.emp_id
                //                 }
        ...
        $scope.criteriaMatchGPS = function(criteria) {
            return function(item) {
                ...
                if ($scope.DriverGPSFilterV == 'Online' && status == 'Online') {
                    return true;
                } else if ($scope.DriverGPSFilterV == 'Inactive' && status == 'Inactive') {
                    return true;
                } else if ($scope.DriverGPSFilterV == 'Offline' && status == 'Offline') {
                    return true;
...
$scope.TripStared = [];
        ...
        $scope.ToggleTrackingView = function(data, type) {
            ...
            else if($scope.ToggleTrackingType == 'emp')
            {
                ...
                var datP = moment($scope.datePicker.startDate).format('YYYY-MM-DD');
                var datB = moment($scope.datePicker.endDate).format('YYYY-MM-DD');


                     $http.get('new_task?start='+datP+'&end='+datB+'&emp='+$scope.TaskViewEmp_id).then(function(data)
                    {

                        console.log($scope.TaskViewEmp_id);
                ...
                $scope.TaskViewList.push(item);
                    ...
                    $http.get('getMyLocations?start='+datP+'&end='+datB+'&emp='+$scope.TaskViewEmp_id).then(function(data)
                    {
