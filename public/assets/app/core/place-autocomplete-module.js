...
angular.module('app')
  .directive('placeAutocomplete', function() {
    return {
      template: '<div id=place-autocomplete><md-autocomplete md-floating-label="Location (place, city or region)"md-item-text=item.description md-items="item in search(searchText)"md-max-length=50 md-min-length=2 md-no-cache=false md-search-text=searchText md-search-text-change=search(searchText) md-selected-item=location md-selected-item-change=getLatLng(item) ng-model-options="{debounce: 600}"><md-item-template><span md-highlight-flags=^i md-highlight-text=searchText>{{item.description}}</span></md-item-template><md-not-found>No matches found for "{{searchText.description}}".</md-not-found></md-autocomplete></div>',
      restrict: 'E',
      replace: true,
      scope: {
        'ngModel': '='
