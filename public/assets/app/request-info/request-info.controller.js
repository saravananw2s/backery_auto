(function() {
    'use strict';
    angular.module('app').controller('RequestCtrl', ['$rootScope', '$state', '$stateParams', '$mdToast', '$timeout', '$scope', '$mdEditDialog', '$q', '$http', '$mdDialog', 'api', RequestCtrl])

    function RequestCtrl($rootScope, $state, $stateParams, $mdToast, $timeout, $scope, $mdEditDialog, $q, $http, $mdDialog, api) {
        ...
        $scope.view = function(id) {
            $state.go('admin.requestinfoview', {
                id: api.encode(id)
            });
        }
        ...
        $scope.removeChoice = function(data) {
            if (data > -1) {
                $scope.data.request_info.splice(data, 1);
            }
        };
        ...
        $scope.update_info = function(dataid) {
            ...
            $http.get($scope.apipath + dataid).then(function(response) {
                ...
                if (res.data.status == 'ok') {
                    ...
                    $scope.data.request_info = angular.fromJson($scope.data.request_info);
